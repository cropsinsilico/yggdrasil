#ifndef CISPLYSERIALIZE_H_
#define CISPLYSERIALIZE_H_
...
int serialize_ply(const seri_t s, char *buf, const size_t buf_size,
  ...
  if (ilen < 0) {
    cislog_error("serialize_ply: Error formatting header.");
    return -1;
  } else if (ilen >= buf_size) {
    cislog_error("serialize_ply: Buffer (size = %d) is not large "
  ...
  for (i = 0; i < p.nvert; i++) {
    ...
    if (ilen < 0) {
      cislog_error("serialize_ply: Error formatting vertex %d.", i);
      return -1;
  ...
  for (i = 0; i < p.nface; i++) {
    ...
    if (ilen < 0) {
      cislog_error("serialize_ply: Error formatting number of verts for face %d.", i);
      return -1;
    } else if (ilen > (buf_size - msg_len)) {
      cislog_error("serialize_ply: Buffer (size = %d) is not large "
    ...
    for (j = 0; j < p.nvert_in_face[i]; j++) {
      ...
      if (ilen < 0) {
        cislog_error("serialize_ply: Error formatting element %d of face %d.", j, i);
        return -1;
      } else if (ilen > (buf_size - msg_len)) {
	cislog_error("serialize_ply: Buffer (size = %d) is not large "
    ...
    if (ilen < 0) {
      cislog_error("serialize_ply: Error formatting newline for face %d.", i);
      return -1;
    } else if (ilen > (buf_size - msg_len)) {
      cislog_error("serialize_ply: Buffer (size = %d) is not large "
...
int deserialize_ply(const seri_t s, const char *buf, const size_t buf_siz,
  ...
  if (out > 0) {
    ...
    if (n_sub_matches < 2) {
      cislog_error("deserialize_ply: Could not locate number of vertices in ply header.");
      out = -1;
    }
  ...
  if (out > 0) {
    ...
    if (n_sub_matches < 2) {
      cislog_error("deserialize_ply: Could not locate number of faces in ply header.");
      out = -1;
    }
    value_size = eind[1] - sind[1];
    memcpy(value, buf + sind[1], value_size);
  ...
  if (out > 0) {
    ...
    if (n_sub_matches < 1) {
      cislog_error("deserialize_ply: Could not locate end of header.");
      out = -1;
    } else {
      begin_body = eind[0];
    }
  ...
  if (out > 0) {
    ...
    if ((nvert + nface) > nlines) {
      cislog_error("deserialize_ply: Not enough lines (%d) for %d vertices "
		   "and %d faces.",
		   nlines, nvert, nface);
      out = -1;
    }
  ...
  if (out > 0) {
    ...
    if (ret < 0) {
      cislog_error("deserialize_ply: Error allocating ply structure.");
      out = -1;
    }
  }
  // Get vertices
  if (out > 0) {
	...
	cislog_error("deserialize_ply: Vertex should contain %d entries. "
  ...
  if (out > 0) {
	...
	cislog_error("deserialize_ply: Face should contain %d entries. "
        ...
        if (iface == NULL) {
          cislog_error("deserialize_ply: Could not allocate face %d.", i);
          out = -1;
          break;
        } else {
          p->faces[i] = iface;
