import pika
import uuid
from cis_interface.drivers.RMQAsyncDriver import RMQAsyncDriver
from cis_interface.drivers.RPCDriver import RPCDriver
from cis_interface.drivers.RMQAsyncServerDriver import (
    _new_client_msg, _end_client_msg)


class RMQAsyncClientDriver(RMQAsyncDriver, RPCDriver):
    ...
    def __init__(self, name, args=None, **kwargs):
        if args is None:
            args = name + '_SERVER'
        super(RMQAsyncClientDriver, self).__init__(name, queue="", **kwargs)
        self.debug()
    ...
    #     self._nacked = 0
    #     self._message_number = 0
    #     super(RMQAsyncClientDriver, self).reconnect()

    # If we force the RMQAsyncServerDriver to start the request queue, it can
    ...
    #     super(RMQAsyncClientDriver, self).on_queue_declareok(method_frame)
    ...
    def start_publishing(self):
        r"""Begin moving messages from IPC queue to RMQAsync queue."""
    ...
    def schedule_next_response(self):
        ...
        if self.response is None:
            self.debug('::Checking RMQAsync response queue again in %5.2f seconds',
                       self.sleeptime)
            with self.lock:
                if not self.channel_stable:  # pragma: debug
                    return
    ...
    def stop_communication(self):
        ...
        super(RMQAsyncClientDriver, self).stop_communication(cancel_consumer=True)

    def on_model_exit(self):
        r"""Explicitly call both RMQAsyncDriver and RPCDriver versions."""
        super(RMQAsyncClientDriver, self).on_model_exit()
