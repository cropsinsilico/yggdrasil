...
class TestRMQAsyncServerParam(parent1.TestRMQAsyncParam, IOInfo):
...
class TestRMQAsyncServerDriver(TestRMQAsyncServerParam, parent1.TestRMQAsyncDriver):
    ...
    def setup(self):
        ...
        super(TestRMQAsyncServerDriver, self).setup()
        self.create_in_rmq()

    def teardown(self):
        r"""Remove the instance, stoppping it."""
        self.destroy_in_rmq()
        super(TestRMQAsyncServerDriver, self).teardown()
    ...
    def test_msg(self):
        ...
        # Send message to RMQAsync input & receive from IPC input
        with self.instance.lock:
            self.instance.channel.basic_publish(
                exchange=self.instance.exchange,
                routing_key=self.instance.queue,
        ...
        # Send message to IPC output & receive from RMQAsync output
