...
%---------------------------------------------------------------------------------
% SOURCE CODE
\newcommand{\rootsrc}{\lstinputlisting[language=C,label={lst:src:root},caption={root.h: Source code for root model in C.}]{../../cis_interface/examples/root_to_shoot/src/root.h}}
\newcommand{\shootsrc}{\lstinputlisting[language=Python,label={lst:src:shoot},caption={shoot.py: Source code for shoot model in Python.}]{../../cis_interface/examples/root_to_shoot/src/shoot.py}}

%---------------------------------------------------------------------------------
% WRAPPER CODE
\newcommand{\rootwrap}{\lstinputlisting[language=C,label={lst:wrap:root},caption={root\_wrapper.c: Wrapper source code for root model in C.}]{../../cis_interface/examples/root_to_shoot/src/root_wrapper.c}}
\newcommand{\shootwrap}{\lstinputlisting[language=Python,label={lst:wrap:shoot},caption={shoot\_wrapper.py: Wrapper source code for shoot model in Python}]{../../cis_interface/examples/root_to_shoot/src/shoot_wrapper.py}}

%---------------------------------------------------------------------------------
% YAML FILES
\newcommand{\rootyml}{\lstinputlisting[label={lst:yml:root},caption={root.yml: Model YAML specification file for root model.}]{../../cis_interface/examples/root_to_shoot/root.yml}}
\newcommand{\shootyml}{\lstinputlisting[label={lst:yml:shoot},caption={shoot.yml: Model YAML specification file for shoot model.}]{../../cis_interface/examples/root_to_shoot/shoot.yml}}
\newcommand{\rootymlfiles}{\lstinputlisting[label={lst:yml:root_files},caption={root\_files.yml: Connection YAML specification file for running the root model in isolation.}]{../../cis_interface/examples/root_to_shoot/root_files.yml}}
\newcommand{\shootymlfiles}{\lstinputlisting[label={lst:yml:shoot_files},caption={shoot\_files.yml: Model YAML specification file for running the shoot model in isolation.}]{../../cis_interface/examples/root_to_shoot/shoot_files.yml}}
\newcommand{\intyml}{\lstinputlisting[label={lst:yml:int},caption={root\_to\_shoot.yml: Connection YAML specification file for the root/shoot integration.}]{../../cis_interface/examples/root_to_shoot/root_to_shoot.yml}}
...
the model source code and let one of several {\pkg} drivers handle the compilation, including 
...
The algorithms for a generic model driver and the master thread can be found in Algorithms \ref{alg:model} \& \ref{alg:master} respectively in \A\ref{A:algorithms}.
...
working in parallel. The procedures for creating and using temporary channels for sending large messages can be seen in the {\sc Send} and {\sc Recv} methods from Algorithms \ref{alg:async_send} \& \ref{alg:async_recv} respectively in \A\ref{A:algorithms}.
...
to more complex data types and communication patterns. Pseudocode describing the asynchronous communication classes and functions underlying the interfaces is provided by Algorithms \ref{alg:async_send} \& \ref{alg:async_recv} in \A\ref{A:algorithms}.
...
\end{equation}
%
For the example presented above, the speedup offered by {\pkg} through parallelism can be parameterized in terms of the durations of calculations for each model (designated model A \& B for brevity) performed in the different phases of the process (See Figure \ref{fig:empty_parallel}). 
%
\ifinclfig
 	\begin{figure}[htbp]
	\begin{center}
...
%
While {\pkg} is a powerful tool for integrating scientific models and opening up new avenues for collaboration, it is not applicable to every scenario.
%
\begin{description}
	\item[Single Language Integrations] For models written in the same language, it is usually much more computationally efficient to either integrate the models directly (i.e. by calling one from the other) or use a language specific tools for parallel execution and passing data \citep[e.g. Parsl in Python][]{babuji18} than to use {\pkg}. This performance advantage will be particularly pronounced for models that finish quickly or tightly coupled integrations where models make a large number of exchanges in rapid succession (e.g. a set of coupled ODEs). Although the constituent models will still benefit from integration via {\pkg} in regards to reusability, {\pkg} may not be the best choice in such cases if execution time is a concern.
	\item[Easily Translatable Models] For relatively simple models ($<100$ lines of code), it may be more efficient for a scientist to translate the model into the language of the model(s) it will be integrated with. {\pkg} is still useful as a tool more making the model accessible and more likely to be reused, but the performance advantage of running codes in the same language may be of greater importance in certain situations as discussed above.
	\item[Unsupported Languages] While {\pkg} currently supports models written in Python, Matlab, C, \& C++, models are written in many other languages. There are plans to expand support for {\pkg} to other languages (see \S\ref{SS:langsupport}), but in the meantime {\pkg} will not be particularly useful for models that are not written in one of the languages already supported.
\end{description}
