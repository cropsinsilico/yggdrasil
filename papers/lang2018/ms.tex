...
To accomplish these objectives, {\pkg} is built upon several well-established open-source software packages along with tools developed explicitly for {\pkg}. {\pkg} currently support models written in Python, Matlab, C, and C++ with 
additional Domain Specific Language (DSL) support for LPy models \citep{Boudon2012}. Support for additional languages is planned for future development (See \S\ref{SS:langsupport}). \todo{Statement on novelty}
...
The algorithms for a generic model driver and the master thread can be found in Algorithms \ref{alg:model} \& \ref{alg:master} respectively.
% MODEL DRIVER
\begin{algorithm}[H]
...
% PAGE BREAK
\begin{algorithm}[H]
	\caption{Continuation of Algorithm \ref{alg:master}}
	\begin{algorithmic}[1]
...
%\end{center}
%
\todo{make sure this is split correctly across pages}
...
% SENDER
\begin{algorithm}[H]
    \caption{Asynchronous communication class for sending messages to an output channel. The procedures {\sc Open, Close, Send, \& IsOpen} are procedures that are implemented by external communication packages and different slightly from one communication mechanism to the next.}
	...
	\State \textbf{Run} \Call{BacklogThread}{} on a new thread
	\item[]
	%
	\ClassMethod{BacklogThread}{}
		\While{\Call{IsOpen}{$x$}}
    			\If{$length(backlog) > 0$}
				\State $flag \gets$ \Call{Send}{$x$, $backlog[0]$}
				\If{$flag$}
	...
	\ClassMethod{SendBacklog}{$msg$}
		\If{\Call{IsOpen}{$x$}}
			\State \textbf{Append} $backlog$, $msg$
		\EndIf
		\State \textbf{Return} \Call{IsOpen}{$x$}
	\EndClassMethod
	\item[]
	%
	\ClassMethod{Close}{}
...
% RECEIVER
\begin{algorithm}[H]
    \caption{Asynchronous communication class for receiving messages from an input channel. The procedures {\sc Open, Close, Recv, IsOpen, \& IsMessageWaiting} are procedures that are implemented by external communication packages and different slightly from one communication mechanism to the next.}
    ...
    \Class{AsyncReceiver}{$address$}
    	\State $backlog \gets []$
	\State $x \gets$ \Call{Open}{$address$}
	\State $thread \gets$ \Call{BacklogThread}{} on a new thread
	\item[]
	%
	\ClassMethod{BacklogThread}{}
		\While{\Call{IsOpen}{$x$}}
    			\If{\Call{IsMessageWaiting}{$x$}}
				\State $flag, msg \gets$ \Call{Recv}{$x$}
				\If{$flag$}
					\State \textbf{Append} $backlog$, $msg$
				\Else
					\State \Call{Close}{$x$}
...
\begin{algorithm}[H]
	...
	\State $thread \gets$ \Call{ConnectionThread}{} on a new thread
	\item[]
	%
	\ClassMethod{ConnectionThread}{}
		\While{true}
		...
		\EndWhile
		%
		\State $input.$\Call{CloseAsync}{}
		\State $output.$\Call{CloseAsync}{}
	\EndClassMethod
	\item[]
	%
	\ClassMethod{Terminate}{}
	...
	%
	\ClassMethod{Stop}{}
		\While{\Call{IsOpen}{$input.x$} and \Call{IsOpen}{$output.x$}}
			\State \textbf{Sleep}
		\EndWhile
		\State \Call{Terminate}{}
	\EndClassMethod
...
%
\subsection{Use Cases \& Limitations}\label{SS:uses}
%
...
%
\todo{Add discussion of limitations}
