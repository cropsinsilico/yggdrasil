...
bool update_header_from_doc(comm_head_t &head, rapidjson::Value &head_doc) {
    ...
    while (strcmp(*n, "") != 0) {
        if (head_doc.HasMember(*n)) {
            ...
            if (len > target_size) {
                ygglog_error(
                            "update_header_from_doc: Size of value for key '%s' (%d) exceeds size of target buffer (%d).",
                            *n, len, target_size);
                return false;
            }
...
void *generic_array_get_scalar(generic_t x, const size_t index,
    ...
    try {
        ...
        ScalarMetaschemaType *item_type = new ScalarMetaschemaType(subtype, precision, new_units,
                                                                   x_obj->get_type()->use_generic());
...
complex_long_double_t generic_array_get_complex_long_double(generic_t x, const size_t index) {
    return ((complex_long_double_t *) generic_array_get_scalar(x, index, "complex",
                                                               8 * sizeof(complex_long_double_t)))[0];
}
...
size_t generic_array_get_1darray(generic_t x, const size_t index,
    ...
    try {
        ...
        OneDArrayMetaschemaType *item_type = new OneDArrayMetaschemaType(subtype, precision, new_length, new_units,
                                                                         x_obj->get_type()->use_generic());
...
size_t generic_array_get_ndarray(generic_t x, const size_t index,
    ...
    try {
        ...
        NDArrayMetaschemaType *item_type = new NDArrayMetaschemaType(subtype, precision, new_shape, new_units,
                                                                     x_obj->get_type()->use_generic());
        size_t nbytes = item_type->nbytes();
        void *new_data = x_obj->get_data_array_item(index, item_type);
        data[0] = (void *) realloc(data[0], nbytes);
...
size_t generic_array_get_ndarray_complex_float(generic_t x, const size_t index, complex_float_t **data,
                                               size_t **shape) {
    return generic_array_get_ndarray(x, index, "complex", 8 * sizeof(complex_float_t), (void **) data, shape);
}
size_t generic_array_get_ndarray_complex_double(generic_t x, const size_t index, complex_double_t **data,
                                                size_t **shape) {
    return generic_array_get_ndarray(x, index, "complex", 8 * sizeof(complex_double_t), (void **) data, shape);
}
size_t generic_array_get_ndarray_complex_long_double(generic_t x, const size_t index, complex_long_double_t **data,
                                                     size_t **shape) {
...
size_t generic_map_get_ndarray_complex_long_double(generic_t x, const char *key, complex_long_double_t **data,
                                                   size_t **shape) {
            ...
            switch (it->second) {
                ...
                case T_ANY: {
                    ...
                    if (!(is_generic_init(*c_gen_value))) {
                        ygglog_throw_error("generic_array_set_item: %s values must be provided as a generic_t object.",
                                           type);
                    }
                    generic_value = (YggGeneric *) (c_gen_value->obj);
                    if (generic_value == NULL) {
                        ygglog_throw_error("generic_array_set_item: %s value is generic wrapper around a NULL object.",
                                           type);
                    }
                    break;
                }
...
int generic_array_set_scalar(generic_t x, const size_t index,
    ...
    try {
        ...
        ScalarMetaschemaType *item_type = new ScalarMetaschemaType(subtype, precision, units,
                                                                   x_obj->get_type()->use_generic());
...
int generic_array_set_1darray(generic_t x, const size_t index,
    ...
    try {
        ...
        OneDArrayMetaschemaType *item_type = new OneDArrayMetaschemaType(subtype, precision, length, units,
                                                                         x_obj->get_type()->use_generic());
        YggGeneric *generic_value = new YggGeneric(item_type, value);
        x_obj->set_data_array_item(index, generic_value);
        delete item_type;
...
int generic_array_set_1darray_long_double(generic_t x, const size_t index, long double *value, const size_t length,
                                          const char *units) {
    return generic_array_set_1darray(x, index, (void *) value, "float", 8 * sizeof(long double), length, units);
}
int generic_array_set_1darray_complex_float(generic_t x, const size_t index, complex_float_t *value,
...
int generic_array_set_1darray_complex_double(generic_t x, const size_t index, complex_double_t *value,
                                             const size_t length, const char *units) {
    return generic_array_set_1darray(x, index, (void *) value, "complex", 8 * sizeof(complex_double_t), length, units);
}
int generic_array_set_1darray_complex_long_double(generic_t x, const size_t index, complex_long_double_t *value,
                                                  const size_t length, const char *units) {
    return generic_array_set_1darray(x, index, (void *) value, "complex", 8 * sizeof(complex_long_double_t), length,
                                     units);
}
...
int generic_array_set_ndarray(generic_t x, const size_t index,
    ...
    try {
        ...
        NDArrayMetaschemaType *item_type = new NDArrayMetaschemaType(subtype, precision, new_shape, units,
                                                                     x_obj->get_type()->use_generic());
...
int generic_array_set_ndarray_long_double(generic_t x, const size_t index, long double *data, const size_t ndim,
...
int generic_array_set_ndarray_complex_float(generic_t x, const size_t index, complex_float_t *data, const size_t ndim,
                                            const size_t *shape, const char *units) {
    return generic_array_set_ndarray(x, index, (void *) data, "complex", 8 * sizeof(complex_float_t), ndim, shape,
                                     units);
}
int generic_array_set_ndarray_complex_double(generic_t x, const size_t index, complex_double_t *data, const size_t ndim,
                                             const size_t *shape, const char *units) {
    return generic_array_set_ndarray(x, index, (void *) data, "complex", 8 * sizeof(complex_double_t), ndim, shape,
                                     units);
}
int generic_array_set_ndarray_complex_long_double(generic_t x, const size_t index, complex_long_double_t *data,
                                                  const size_t ndim, const size_t *shape, const char *units) {
    return generic_array_set_ndarray(x, index, (void *) data, "complex", 8 * sizeof(complex_long_double_t), ndim, shape,
                                     units);
}
int generic_array_set_ndarray_bytes(generic_t x, const size_t index, char **data, const size_t ndim,
                                    const size_t *shape, const char *units) {
    return generic_array_set_ndarray(x, index, (void *) data, "bytes", 0, ndim, shape, units);
}
int generic_array_set_ndarray_unicode(generic_t x, const size_t index, char **data, const size_t ndim,
                                      const size_t *shape, const char *units) {
        ...
        if (it != type_map.end()) {
            switch (it->second) {
                ...
                case T_ANY: {
                    ...
                    generic_t *c_gen_value = (generic_t *) (value);
                    if (!(is_generic_init(*c_gen_value))) {
                        ygglog_throw_error("generic_map_set_item: %s values must be provided as a generic_t object.",
                                           type);
                    }
                    generic_value = (YggGeneric *) (c_gen_value->obj);
                    if (generic_value == NULL) {
                        ygglog_throw_error("generic_map_set_item: %s value is generic wrapper around a NULL object.",
                                           type);
                    }
                    break;
                }
...
int generic_map_set_1darray_long_double(generic_t x, const char *key, long double *value, const size_t length,
                                        const char *units) {
    return generic_map_set_1darray(x, key, (void *) value, "float", 8 * sizeof(long double), length, units);
}
int generic_map_set_1darray_complex_float(generic_t x, const char *key, complex_float_t *value, const size_t length,
                                          const char *units) {
    return generic_map_set_1darray(x, key, (void *) value, "complex", 8 * sizeof(complex_float_t), length, units);
}
int generic_map_set_1darray_complex_double(generic_t x, const char *key, complex_double_t *value, const size_t length,
                                           const char *units) {
    return generic_map_set_1darray(x, key, (void *) value, "complex", 8 * sizeof(complex_double_t), length, units);
}
int generic_map_set_1darray_complex_long_double(generic_t x, const char *key, complex_long_double_t *value,
                                                const size_t length, const char *units) {
...
int generic_map_set_ndarray_long_double(generic_t x, const char *key, long double *data, const size_t ndim,
                                        const size_t *shape, const char *units) {
    return generic_map_set_ndarray(x, key, (void *) data, "float", 8 * sizeof(long double), ndim, shape, units);
}
int generic_map_set_ndarray_complex_float(generic_t x, const char *key, complex_float_t *data, const size_t ndim,
                                          const size_t *shape, const char *units) {
    return generic_map_set_ndarray(x, key, (void *) data, "complex", 8 * sizeof(complex_float_t), ndim, shape, units);
}
int generic_map_set_ndarray_complex_double(generic_t x, const char *key, complex_double_t *data, const size_t ndim,
                                           const size_t *shape, const char *units) {
    return generic_map_set_ndarray(x, key, (void *) data, "complex", 8 * sizeof(complex_double_t), ndim, shape, units);
}
int generic_map_set_ndarray_complex_long_double(generic_t x, const char *key, complex_long_double_t *data,
                                                const size_t ndim, const size_t *shape, const char *units) {
    return generic_map_set_ndarray(x, key, (void *) data, "complex", 8 * sizeof(complex_long_double_t), ndim, shape,
                                   units);
}
int generic_map_set_ndarray_bytes(generic_t x, const char *key, char **data, const size_t ndim, const size_t *shape,
                                  const char *units) {
    return generic_map_set_ndarray(x, key, (void *) data, "bytes", 0, ndim, shape, units);
}
int generic_map_set_ndarray_unicode(generic_t x, const char *key, char **data, const size_t ndim, const size_t *shape,
                                    const char *units) {
...
dtype_t *create_dtype_json_object(const size_t nitems, char **keys,
    ...
    try {
        ...
        if ((nitems > 0) && ((keys == NULL) || (values == NULL))) {
            ygglog_throw_error(
                    "create_dtype_json_object: %d items expected, but the keys and/or values parameter is NULL.",
                    nitems);
        }
        for (i = 0; i < nitems; i++) {
...
int update_dtype_from_generic_ap(dtype_t *dtype1, size_t nargs,
    ...
    try {
        ...
        if (!(is_generic_init(gen_arg))) {
            ygglog_throw_error(
                    "update_dtype_from_generic_ap: Type expects generic object, but provided object is not generic.");
        } else {
            dtype_t dtype2;
