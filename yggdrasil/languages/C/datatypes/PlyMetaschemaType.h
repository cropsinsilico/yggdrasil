...
public:
  ...
  bool encode_data(rapidjson::Writer<rapidjson::StringBuffer> *writer,
    ...
    // Format header
    char header_format[1000] = "ply\n"
      "format ascii 1.0\n"
      "comment author ygg_auto\n"
      "comment File generated by yggdrasil\n";
    if (strlen(p.material) != 0) {
      char header_format_material[100];
      ilen = snprintf(header_format_material, 100,
		      "comment material: %s\n", p.material);
      if ((ilen < 0) || (ilen >= 100)) {
	ygglog_error("PlyMetaschemaType::encode_data: Error formatting material.");
	return false;
      }
      strcat(header_format, header_format_material);
    }
    char header_format1[500] = "element vertex %d\n"
      "property float x\n"
      "property float y\n"
      "property float z\n";
    strcat(header_format, header_format1);
    if (p.vertex_colors != NULL) {
      char header_format_colors[100] = "property uchar red\n"
    ...
    char header_edges[100] = "element edge %d\n"
      "property int vertex1\n"
      "property int vertex2\n";
    strcat(header_format, header_edges);
    if (p.edge_colors != NULL) {
      char header_format_edge_colors[100] = "property uchar red\n"
	"property uchar green\n"
	"property uchar blue\n";
      strcat(header_format, header_format_edge_colors);
    }
    char header_format_end[100] = "end_header\n";
    strcat(header_format, header_format_end);
    while (true) {
      ilen = snprintf(buf, buf_size, header_format, p.nvert, p.nface, p.nedge);
      if (ilen < 0) {
    ...
    for (i = 0; i < p.nedge; i++) {
      while (true) {
	if (p.edge_colors != NULL) {
	  ilen = snprintf(buf + msg_len, buf_size - msg_len, "%d %d %d %d %d\n",
			  p.edges[i][0], p.edges[i][1],
			  p.edge_colors[i][0], p.edge_colors[i][1], p.edge_colors[i][2]);
	} else {
	  ilen = snprintf(buf + msg_len, buf_size - msg_len, "%d %d\n",
			  p.edges[i][0], p.edges[i][1]);
	}
	if (ilen < 0) {
