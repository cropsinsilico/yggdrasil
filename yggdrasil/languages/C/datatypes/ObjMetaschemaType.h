...
public:
    ...
    bool encode_data(rapidjson::Writer<rapidjson::StringBuffer> *writer,
        ...
        if (p.texcoords == NULL) {
            if (p.ntexc > 0) {
                ygglog_error("ObjMetaschemaType::encode_data: There are %d texcoords, but the vector is NULL.",
                             p.ntexc);
                return false;
            }
        } else {
        ...
        if (p.curves2 == NULL) {
            if (p.ncurve2 > 0) {
                ygglog_error("ObjMetaschemaType::encode_data: There are %d curve2s, but the vector is NULL.",
                             p.ncurve2);
                return false;
            }
        } else {
        ...
        try {
            ...
            if (p->surface_texcoords != NULL) {
                ...
                if (isurf_texcoord == NULL) {
                    ygglog_throw_error("ObjMetaschemaType::decode_surface: Failed to allocate surface texcoord %d.",
                                       csurf);
                }
            ...
            if (p->surface_normals != NULL) {
                ...
                if (isurf_normal == NULL) {
                    ygglog_throw_error("ObjMetaschemaType::decode_surface: Failed to allocate surface normal %d.",
                                       csurf);
                }
            ...
            while ((cur_pos < buf_siz) && (out >= 0)) {
                ...
                if (find_matches("#[^\n]*", iline, &sind, &eind) == 1) {
                    ...
                    if (nvert_found != nvert_local) {
                        ygglog_error(
                                    "deserialize_obj: Expected %d verts in point, but found %d (re = %s, line = '%s').",
