...
public:
  ...
  YggGeneric* python2c(PyObject *pyobj) const override {
    ...
    ygglog_throw_error("ObjMetaschemaType::python2c: Python disabled");
  ...
  PyObject* c2python(YggGeneric *cobj) const override {
    ...
    PyObject *pyobj = NULL;
#ifdef YGGDRASIL_DISABLE_PYTHON_C_API
    ygglog_throw_error("ObjMetaschemaType::c2python: Python disabled");
#else // YGGDRASIL_DISABLE_PYTHON_C_API
  ...
  bool encode_data(rapidjson::Writer<rapidjson::StringBuffer> *writer,
    ...
    obj_t p;
    if (ap.using_ptrs) {
      p = ((obj_t*)get_va_list_ptr_cpp(&ap))[0];
    } else {
	...
	char ival[100];
	snprintf(iline, 500, "p");
	for (j = 0; j < p.nvert_in_point[i]; j++) {
	  snprintf(ival, 100, " %d", p.points[i][j] + 1);
	  strcat(iline, ival);
	}
	while (true) {
	...
	return false;
      ...
      for (i = 0; i < p.nline; i++) {
	char ival[10];
	snprintf(iline, 500, "l");
	for (j = 0; j < p.nvert_in_line[i]; j++) {
	  snprintf(ival, 10, " %d", p.lines[i][j] + 1);
	  strcat(iline, ival);
	  if (p.line_texcoords != NULL) {
	    if (p.line_texcoords[i][j] >= 0) {
	      snprintf(ival, 10, "/%d", p.line_texcoords[i][j] + 1);
	      strcat(iline, ival);
	    }
	  }
	}
	...
	char ival[10];
	snprintf(iline, 500, "curv");
	for (j = 0; j < 2; j++) {
	  snprintf(ival, 10, " %f", p.curve_params[i][j]);
	  strcat(iline, ival);
	}
	for (j = 0; j < p.nvert_in_curve[i]; j++) {
	  snprintf(ival, 10, " %d", p.curves[i][j] + 1);
	  strcat(iline, ival);
	}
	while (true) {
	  ilen = snprintf(buf + msg_len, buf_size - msg_len, "%s\n", iline);
	...
	char ival[10];
	snprintf(iline, 500, "curv2");
	for (j = 0; j < p.nparam_in_curve2[i]; j++) {
	  snprintf(ival, 10, " %d", p.curves2[i][j] + 1);
	  strcat(iline, ival);
	}
	while (true) {
	  ilen = snprintf(buf + msg_len, buf_size - msg_len, "%s\n", iline);
	...
	return false;
      ...
      for (i = 0; i < p.nsurf; i++) {
	char ival[10];
	snprintf(iline, 500, "surf");
	for (j = 0; j < 2; j++) {
	  snprintf(ival, 10, " %f", p.surface_params_u[i][j]);
	  strcat(iline, ival);
	}
	for (j = 0; j < 2; j++) {
	  snprintf(ival, 10, " %f", p.surface_params_v[i][j]);
	  strcat(iline, ival);
	}
	for (j = 0; j < p.nvert_in_surface[i]; j++) {
	  snprintf(ival, 10, " %d", p.surfaces[i][j] + 1);
	  strcat(iline, ival);
	  strcat(iline, "/");
	  if (p.surface_texcoords != NULL) {
	    if (p.surface_texcoords[i][j] >= 0) {
	      snprintf(ival, 10, "%d", p.surface_texcoords[i][j] + 1);
	      strcat(iline, ival);
	    }
	  }
	  strcat(iline, "/");
	  if (p.surface_normals != NULL) {
	    if (p.surface_normals[i][j] >= 0) {
	      snprintf(ival, 10, "%d", p.surface_normals[i][j] + 1);
	      strcat(iline, ival);
	    }
	  }
	}
  ...
  bool decode_data(rapidjson::Value &data, const int allow_realloc,
	...
	ygglog_throw_error("ObjMetaschemaType::decode_data: could not realloc pointer.");
      ...
      *p = init_obj();
      ...
      if (ap.using_ptrs) {
	p = (obj_t*)get_va_list_ptr_cpp(&ap);
      } else {
	p = va_arg(ap.va, obj_t*);
      }
    }
