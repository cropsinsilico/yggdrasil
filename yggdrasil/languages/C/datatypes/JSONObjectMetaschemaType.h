...
public:
    ...
    */
     ...
     */
    JSONObjectMetaschemaType *copy() const override {
        return (new JSONObjectMetaschemaType(properties_, use_generic()));
    }
    ...
    */
    ...
    void update_properties(const MetaschemaTypeMap new_properties,
        ...
        if (properties_.size() > 0) {
            ...
            for (it = new_properties.begin(); it != new_properties.end(); it++) {
                if (it->second == NULL) {
                    ygglog_throw_error(
                            "JSONObjectMetaschemaType::update_properties: New value for property '%s' is NULL.",
                            it->first.c_str());
                } else {
    ...
    const MetaschemaType *get_property_type(const char *key) const override {
        ...
        if (it == properties_.end()) {
            ygglog_throw_error("JSONObjectMetaschemaType::get_property_type: Could not locate property for key '%s'",
                               key);
        }
        out = it->second;
        return out;
    ...
    PyObject *c2python(YggGeneric *cobj) const override {
        ...
        if (c_map.size() != nitems()) {
            ygglog_throw_error("JSONObjectMetaschemaType::c2python: Type has %lu elements but object has %lu.",
                               nitems(), c_map.size());
        }
        MetaschemaTypeMap::const_iterator it;
        for (it = properties_.begin(); it != properties_.end(); it++) {
