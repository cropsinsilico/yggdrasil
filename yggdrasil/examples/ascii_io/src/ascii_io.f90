program main
  ...
  allocate(character(len=line_size) :: line)
  ...
  character(len=:), dimension(:), allocatable, target :: name_arr
  ! TODO: Check that these types are equivalent
  integer, dimension(:), allocatable, target :: number_arr
  real, dimension(:), allocatable, target :: value_arr
  complex, dimension(:), allocatable, target :: comp_arr
  integer :: i
  ...
  ret = 0
  do while (ret.ge.0)
     ret = ygg_recv_realloc(array_input, &
  ...
  if (allocated(name_arr)) deallocate(name_arr)
  if (allocated(number_arr)) deallocate(number_arr)
  if (allocated(value_arr)) deallocate(value_arr)
  if (allocated(comp_arr)) deallocate(comp_arr)
