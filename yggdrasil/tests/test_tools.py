...
class TestYggThread(YggTestClass):
    r"""Test basic behavior of YggThread class."""

    _cls = 'YggThread'
    _mod = 'yggdrasil.tools'

    def __init__(self, *args, **kwargs):
        super(TestYggThread, self).__init__(*args, **kwargs)
        self.namespace = 'TESTING_%s' % self.uuid
        self.attr_list += ['name', 'sleeptime', 'longsleep', 'timeout']
        self._inst_kwargs = {'timeout': self.timeout,
                             'sleeptime': self.sleeptime}
...
class TestYggProcess(TestYggThread):
    r"""Test basic behavior of YggThread for spawned process."""
...
class TestYggProcessFork(TestYggProcess):
    r"""Test basic behavior of YggThread for forked process."""

    def __init__(self, *args, **kwargs):
        super(TestYggProcessFork, self).__init__(*args, **kwargs)
        self._inst_kwargs['context'] = tools.mp_ctx
