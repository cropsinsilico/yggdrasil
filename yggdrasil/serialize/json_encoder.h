...
public:
  ...
  bool String   (const Ch* str, SizeType len, bool copy) {
	...
	break;
      ...
      if (target) {
	if (len > size_t) {
	  ygglog_error("HeadDocumentEncoder: Size of value for key '%s' (%d) exceeds size of target buffer (%d).",
...
public:
  ...
  bool StartScalar() {
    ...
    if (level_ > 1) {
      ygglog_error("BodyEncoder: Maximum level of 1 imposed to prevent arbitrary objects.");
      return false;
    }
    if ((level_ == 1) and (not in_array_.top())) {
      ygglog_error("BodyEncoder: Only flat arrays are allowed.");
      return false;
    }
    if (in_array_.top()) {
	...
	if (array_element_count_.top() > curr_type_.top().Size()) {
	  ygglog_error("BodyEncoder: Array does not have an element %d." % array_element_count_.top());
	  return false;
	}
	if (not curr_type_.top()[array_element_count_.top()].IsObject()) {
	  ygglog_error("BodyEncoder: Type for element %d is not an object." % array_element_count_.top());
	  return false;
	}
	curr_type_.push(curr_type_.top()[array_element_count_.top()].GetObject());
      } else {
	ygglog_error("BodyEncoder: Array type must be an object or array of types.");
	return false;
      }
    }
    if (!(curr_type_.top().IsObject())) {
      ygglog_error("BodyEncoder: Type definition must be an object.");
      return false;
    }
    if (not curr_type_.top().HasMember("type")) {
      ygglog_error("BodyEncoder: Type object does not have explicit type entry.");
      return false;
    }
    if (not curr_type_.top()["type"].IsString()) {
      ygglog_error("BodyEncoder: Type is not a string.");
      return false;
    }
    in_array.push(false);
    return true;
  ...
  bool String   (const Ch* str, SizeType len, bool copy) {
	...
	if (not (curr_type_.top().HasMember("length"))) {
	  ygglog_error("BodyEncoder: 1darray types must include 'length'.");
	  return false;
	}
	if (not (curr_type_.top()["length"].IsInt())) {
	  ygglog_error("BodyEncoder: 1darray 'length' value must be an int.");
	  return false;
	}
	nele = (size_t)curr_type_.top()["length"].GetInt();
      } else {
	if (not (curr_type_.top().HasMember("shape"))) {
	  ygglog_error("BodyEncoder: ndarray types must include 'shape'.");
	  return false;
	}
	if (not (curr_type_.top()["shape"].IsArray())) {
	  ygglog_error("BodyEncoder: ndarray 'shape' value must be an int.");
	  return false;
	}
	...
	for (rapidjson::Value::ConstValueIterator itr = a.Begin(); itr != a.End(); ++itr) {
	  if (not (itr.IsInt())) {
	    ygglog_error("BodyEncoder: All elements in shape must be intengers.");
	    return false;
	  }
	...
	ygglog_error("BodyEncoder: Precision missing.");
	return false;
      }
      if (not (curr_type_.top()["precision"].IsInt())) {
	ygglog_error("BodyEncoder: Precision must be integer.");
	return false;
      }
	...
	ygglog_error("BodyEncoder: %d bytes were expected, but %d were decoded.",
		     nbytes, decoded_len);
	return false;
      }
      ...
      if (t2 == NULL) {
	ygglog_error("BodyEncoder: Failed ro realloc temp var.");
	free(*temp);
	return false;
      }
      *temp = t2;
  ...
  bool StartObject() {
    ...
    if (not curr_type_.top().HasMember("properties")) {
      ygglog_error("BodyEncoder: Object type def does not have 'properties' defined.");
      return false;
    }
    if (not curr_type_.top()["properties"].IsObject()) {
      ygglog_error("BodyEncoder: Properties value is not an object.");
      return false;
    }
  ...
  bool Key(const Ch* str, SizeType len, bool copy) {
    ...
    if (not curr_type_.top().HasMember(str)) {
      ygglog_error("BodyEncoder: There is not a property definition for key '%s'.", str);
      return false;
    }
  ...
  bool StartArray() {
    ...
    if (not curr_type_.top().HasMember("items")) {
      ygglog_error("BodyEncoder: Object type def does not have 'items' defined.");
      return false;
    }
